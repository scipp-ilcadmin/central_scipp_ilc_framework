...
public class BeamcalEnergyDep_c extends Driver {
    ...
    *******************************************************************************************/
	...
	try {
		...
		if ( reject_negative && (vec[2]<0) ){ layersHit2.add(layer);} //Pass over negative Bcal hits
                else {
		    double energy = hit.getRawEnergy();		   
		    sumOfEnergy += energy;

		    if(maxPixelEnergy < energy){
			maxPixelEnergy = energy; layerOfMaxE = layer;
		    }
		    ...
		    //layer specific plots (iff layerBcal ==true)
		    if(layersBcal && layer <= 15){
			power[layer] += powerDraw(radDosage(energy/numberOfEvents),runTemp);//Divide by #Evs for avg/event
			...
			//More plots (like for radDosage/Edep would go here)
		    }
