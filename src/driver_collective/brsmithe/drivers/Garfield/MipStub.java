...
final public class MipStub implements Cluster {
  ...
  // Base of Mipstub
    public double[] base;
    // Direction of mipstub
    public double[] dir;
    public double kappa;  // curvature estimate
    private double baseError; // Uncertainty of the base location?
    private double dirXYError; // Uncertainty of the direction in the XY plane? 
    private double dirZError; // Uncertainy in its direciton in Z
    private double kappaError; // curvature uncertainty
    // If the angle made from the Z axis is too extreme, its not on the barrel.
    private boolean isEndcap = false; 
    public double energy;  // energy estimate
    private double energyError; //Our estimate by how far off our estimate is
    ...
    private Cluster cluster = null; //The cluster we're working with
